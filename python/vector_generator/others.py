...
def generate_recover_vectors():
    ...
    invalid_crec = crec[1:]
    error = expect_exception(
        lambda: recover(None, invalid_crec),
        chilldkg.RecoveryDataError
    )
    vectors["error_test_cases"].append({
        "hostseckey": None,
    ...
    rand_encshare = bytes.fromhex("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141")
    ...
    n = len(hostpubkeys)
    cert_len = chilldkg.certeq_cert_len(n)
    rand_ge = bytes.fromhex("03421F5FC9A21065445C96FDB91C0C1E2F2431741C72713B4B99DDCB316F31E9FC")
    invalid_crec = crec[:-cert_len - 32 * n - 33] + rand_ge + crec[-cert_len - 32 * n:]
    error = expect_exception(
        lambda: recover(None, invalid_crec),
        chilldkg.RecoveryDataError
    )
    ...
    rand_sig = bytes.fromhex("09C289578B96E6283AB13E4741FB489FC147FB1A5F446A314BA73C052131EFB04B83247A0BCEDF5205202AD64188B24B0BC5B51A17AEB218BD98DBE000C843B9")
    ...
    assert len(short_hostseckey) == 16
    error = expect_exception(
        lambda: recover(short_hostseckey, crec),
        chilldkg.HostSeckeyError
    )
    vectors["error_test_cases"].append({
        "hostseckey": bytes_to_hex(short_hostseckey),
    ...
    error = expect_exception(
        lambda: recover(rand_hostseckey, crec),
        chilldkg.HostSeckeyError
    )
    vectors["error_test_cases"].append({
